...
async function testSimplePriozerie(): Promise<void> {
  ...
  try {
    ...
    if (quickResults.length > 0) {
      ...
      console.log(
        "  Данные:",
        firstResult.columns.filter((col) => col.trim())
      );
    }
 ...
 */
async function saveSimpleResults(rows: any[]): Promise<void> {
  try {
    const { promises: fs } = await import("fs");

    await fs.mkdir("reports/test-results", { recursive: true });

    const filename = `priozerie-simple-${
    ...
    console.error("⚠️ Ошибка сохранения:", (error as Error).message);
  }
}
