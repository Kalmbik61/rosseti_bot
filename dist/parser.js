...
import { logger, buildSearchUrl, MY_PLACE, findMyPlaceInRow, findPriozeryeInRow, extractPowerOutageInfo, ParserCallLogger, } from "./utils/index.js";
/**
 * Главная функция поиска ${MY_PLACE}
 * @param url - URL для поиска
 * @returns Результат с найденными строками
 */
export async function findPriozeryeInTables(url) {
    const callLogger = new ParserCallLogger("findPriozeryeInTables", url, "GET", {}, MY_PLACE);
    ...
    try {
        ...
        logger.info(`Поиск завершен. Найдено ${result.matchesFound} строк с ${MY_PLACE}`);
        ...
        // Логируем ошибку
        await callLogger.logError(error);
        throw error;
    }
    finally {
        await browser.close();
 ...
 */
async function searchPriozeryeInPage(page) {
    ...
    for (const [tableIndex, table] of tables.entries()) {
        ...
        for (const [rowIndex, row] of rows.entries()) {
            ...
            if (priozeryeMatch) {
                ...
                logger.debug(`Найдено ${priozeryeMatch.variant} в таблице ${tableIndex + 1}, строке ${rowIndex + 1}, колонке ${priozeryeMatch.columnIndex + 1}`);
            }
        }
    }
    return {
 ...
 */
export async function findPowerOutages(url) {
    const callLogger = new ParserCallLogger("findPowerOutages", url, "GET", {}, MY_PLACE);
    const browser = await chromium.launch({
        headless: true,
        args: ["--no-sandbox", "--disable-setuid-sandbox"],
    });
    ...
    try {
        ...
        logger.info(`Найдено ${powerOutages.length} записей с отключениями для ${MY_PLACE}`);
        // Логируем успешный результат
 ...
 */
async function searchPowerOutagesInPage(page) {
    const tables = await page.$$("table");
    logger.info(`Найдено таблиц: ${tables.length}`);
    const powerOutages = [];
    for (const [tableIndex, table] of tables.entries()) {
        ...
        for (const row of rows) {
            ...
            if (placeColumnIndex !== null) {
                ...
                logger.debug(`Найдено отключение для ${MY_PLACE}: ${JSON.stringify(outageInfo)}`);
            }
        }
    }
    return powerOutages;
 ...
 */
export async function quickFindPriozerie(days = 30) {
    const today = new Date();
    const dateFrom = new Date(today);
    dateFrom.setDate(today.getDate() - days);
    const url = buildSearchUrl(dateFrom, today);
    const callLogger = new ParserCallLogger("quickFind", url, "GET", { days }, MY_PLACE);
    try {
 ...
 */
export async function quickFindPowerOutages(days = 30) {
    const today = new Date();
    const dateFrom = new Date(today);
    dateFrom.setDate(today.getDate() - days);
    const url = buildSearchUrl(dateFrom, today);
