  ...
  private async setupBotMenu(): Promise<void> {
    try {
      ...
      const adminCommands = [
        ...
        {
          command: "admin_backup_list",
          description: "📋 Список бэкапов",
        },
        {
          command: "admin_backup_restore",
          description: "🔄 Восстановить из бэкапа",
        },
      ];
    ...
    this.bot.on("callback_query", async (callbackQuery) => {
      ...
      try {
        if (data?.startsWith("unsubscribe_")) {
          ...
          await this.handleBackupRestoreCallback(callbackQuery);
        } else {
          await this.bot.answerCallbackQuery(callbackQuery.id, {
...
`;
  ...
  /**
   * Обработка callback для восстановления бэкапа
   */
  private async handleBackupRestoreCallback(
    callbackQuery: TelegramBot.CallbackQuery
  ): Promise<void> {
    const chatId = callbackQuery.message?.chat.id;
    const data = callbackQuery.data;
    ...
    try {
      if (data === "restore_backup_cancel") {
        // Отмена восстановления
        await this.bot.answerCallbackQuery(callbackQuery.id, {
          text: "❌ Восстановление отменено",
        });
      ...
      // Извлекаем индекс бэкапа
      const backupIndex = parseInt(data.replace("restore_backup_", ""), 10);

      if (isNaN(backupIndex)) {
        await this.bot.answerCallbackQuery(callbackQuery.id, {
          text: "❌ Неверный индекс бэкапа",
          show_alert: true,
        });
        return;
      }
      ...
      logger.info(
        `Admin: Восстановление бэкапа #${backupIndex} от админа ${callbackQuery.from.id}`
      );
      ...
      if (backupIndex >= validBackups.length) {
        await this.bot.answerCallbackQuery(callbackQuery.id, {
      ...
      const selectedBackup = validBackups[backupIndex];
      ...
      await this.bot.answerCallbackQuery(callbackQuery.id, {
        text: `🔄 Восстановление из ${selectedBackup.filename}...`,
      });

      // Обновляем сообщение - показываем процесс восстановления
      await this.bot.editMessageText(
        `🔄 *Восстановление базы данных*\n\n⏳ Восстанавливаем из бэкапа: \`${escapeMarkdown(
      ...
      await backupManager.restoreFromBackup(selectedBackup.path);

      // Получаем статистику после восстановления
      const dbSize = await backupManager.getDatabaseSize();
      const restoreTime = new Date().toLocaleString("ru-RU");

      // Успешное завершение
      const successMessage = `✅ *База данных восстановлена*

📁 **Восстановлен бэкап:**
• Файл: \`${escapeMarkdown(selectedBackup.filename)}\`
• Размер: ${this.formatFileSize(selectedBackup.size)}
• Дата создания: ${selectedBackup.createdAt.toLocaleString("ru-RU")}

💾 **Текущая БД:**
• Размер: ${this.formatFileSize(dbSize)}
• Время восстановления: ${restoreTime}

⚠️ **Важно:** Все данные с момента создания бэкапа утеряны\\.`;
      ...
      logger.error("Admin: Ошибка восстановления бэкапа:", error);
      ...
      await this.bot.editMessageText(
        `❌ *Ошибка восстановления*\n\nПроизошла ошибка при восстановлении из бэкапа:\n\`${escapeMarkdown(
...
`;
  ...
  /**
   * Админская команда: создание бэкапа базы данных
   */
  private async handleAdminBackupCommand(
    msg: TelegramBot.Message
  ): Promise<void> {
    const chatId = msg.chat.id;

    try {
      logger.info(`Admin: Запрос создания бэкапа от админа ${msg.from?.id}`);
...
• /admin\\_backup\\_restore - восстановление из бэкапа`;
      ...
      logger.error("Telegram: Ошибка при создании бэкапа:", error);
  ...
  /**
   * Админская команда: список бэкапов
   */
  private async handleAdminBackupListCommand(
    msg: TelegramBot.Message
  ): Promise<void> {
    const chatId = msg.chat.id;

    try {
      logger.info(`Admin: Запрос списка бэкапов от админа ${msg.from?.id}`);
      ...
      if (backups.length === 0) {
        await this.bot.sendMessage(
          chatId,
          "📋 *Список бэкапов*\n\n📭 Бэкапы не найдены\\.\n\nИспользуйте /admin_backup для создания первого бэкапа\\.",
          { parse_mode: "MarkdownV2" }
        );
        return;
      }

      let message = `📋 *Список бэкапов*\n\n`;
      message += `💾 **Текущая БД:** ${this.formatFileSize(dbSize)}\n`;
      message += `📦 **Общий размер бэкапов:** ${this.formatFileSize(
        backupsSize
      )}\n\n`;
      ...
      if (validBackups.length > 0) {
        message += `✅ **Валидные бэкапы (${validBackups.length}):**\n`;
        validBackups.forEach((backup, index) => {
          const fileName = escapeMarkdown(backup.filename);
          const size = this.formatFileSize(backup.size);
          const date = backup.createdAt.toLocaleString("ru-RU");
      ...
      if (invalidBackups.length > 0) {
        message += `❌ **Поврежденные бэкапы (${invalidBackups.length}):**\n`;
        invalidBackups.forEach((backup, index) => {
          const fileName = escapeMarkdown(backup.filename);
          message += `${index + 1}\\. \`${fileName}\` \\(поврежден\\)\n`;
        });
      }
      ...
      logger.error("Telegram: Ошибка при получении списка бэкапов:", error);

      await this.bot.sendMessage(
        chatId,
        `❌ Произошла ошибка при получении списка бэкапов:\n\`${
          (error as Error).message
        }\``,
        { parse_mode: "Markdown" }
      );
  ...
  /**
   * Админская команда: восстановление из бэкапа
   */
  private async handleAdminBackupRestoreCommand(
    msg: TelegramBot.Message
  ): Promise<void> {
    const chatId = msg.chat.id;
    ...
    try {
      logger.info(
        `Admin: Запрос восстановления из бэкапа от админа ${msg.from?.id}`
      );
      ...
      if (validBackups.length === 0) {
        await this.bot.sendMessage(
          chatId,
          "📋 *Восстановление из бэкапа*\n\n❌ Нет доступных бэкапов для восстановления\\.\n\nИспользуйте /admin_backup для создания бэкапа\\.",
          { parse_mode: "MarkdownV2" }
        );
        return;
      }

      let message = `🔄 *Восстановление из бэкапа*\n\n`;
      message += `⚠️ **ВНИМАНИЕ:** Это действие заменит текущую базу данных\\!\n\n`;
      message += `📋 **Доступные бэкапы:**\n`;

      // Создаем inline кнопки для выбора бэкапа
      const keyboard = {
        inline_keyboard: [] as any[][],
      };

      validBackups.slice(0, 10).forEach((backup, index) => {
        const fileName = backup.filename;
        const size = this.formatFileSize(backup.size);
        const date = backup.createdAt.toLocaleDateString("ru-RU");

        message += `${index + 1}\\. \`${escapeMarkdown(fileName)}\`\n`;
        message += `   📁 ${size} • 📅 ${date}\n\n`;

        keyboard.inline_keyboard.push([
          {
            text: `${index + 1}. ${fileName} (${size})`,
            callback_data: `restore_backup_${index}`,
          },
        ]);
      });
      ...
      keyboard.inline_keyboard.push([
        {
          text: "❌ Отмена",
          callback_data: "restore_backup_cancel",
        },
      ]);

      message += `👆 **Выберите бэкап для восстановления:**`;
      ...
      logger.error("Telegram: Ошибка при подготовке восстановления:", error);

      await this.bot.sendMessage(
        chatId,
        `❌ Произошла ошибка при подготовке восстановления:\n\`${
          (error as Error).message
        }\``,
        { parse_mode: "Markdown" }
      );
